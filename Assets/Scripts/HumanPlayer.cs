...
public class HumanPlayer : MonoBehaviour, PlayerInterface {
    ...
    public bool skipStatus { //czy gracz ma zostać pominięty
		get{return skip; }
		set{ skip = value; }
	}
    ...
    public void turn() {
		...
		drew = false;
		int i = 0;
		foreach (Card x in handList) { //dla każdej karty na ręce
			GameObject temp = null;
			if (GameObject.Find("Control").GetComponent<Control>().playerHand.transform.childCount > i) //czy karta już jest czy trzeba załadować
			...
			if (handList[i].Equals(Control.discard[Control.discard.Count - 1]) || handList[i].getNumb() >= 13) { //jeśli można zagrać karty
				setListeners(i, temp);
			}
			else {
				temp.transform.GetChild(3).gameObject.SetActive(true); //jeśli nie to zaciemnij
			}
			i++;
		}
	}
    ...
    public void addCards(Card c) { //dodaje kartę do ręki
		handList.Add(c);
	}
	...
	public int getCardsLeft() { //ile kart ma gracz
